###################################################
# Configuration
###################################################

TCEFORM.tt_content {
    header.types.fluidcontent_content.disabled = 1
    header_layout.types.fluidcontent_content.disabled = 1
    header_position.types.fluidcontent_content.disabled = 1
    date.types.fluidcontent_content.disabled = 1
    header_link.types.fluidcontent_content.disabled = 1
    subheader.types.fluidcontent_content.disabled = 1
    layout.types.fluidcontent_content.disabled = 1
    spaceBefore.types.fluidcontent_content.disabled = 1
    spaceAfter.types.fluidcontent_content.disabled = 1
    section_frame.types.fluidcontent_content.disabled = 1
}
